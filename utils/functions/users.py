...
class UserFunctions(object):
    ...
    async def verify_user(cls, user: Member):
        ...
        await log.send(content=f" {user.mention}  **Make sure to go to <#{cls.bot.config['channels']['roles']}> to choose what areas of the server you get access to!**", embed=utils.Embed(color=randint(1, 0xffffff), title=f"{user.name} has joined Serpent's Garden."))
